...
Our artifact makes this tactile: a filing cabinet (KV `Map`, grows per pair) beside a whiteboard (`slot = hash % N`, last-writer-wins). At 120 pairs / 32 slots collisions already appear; flooding to 500→16 craters whiteboard recall while KV stays 100% and its slot-count line climbs against the flat O(1) line. Clicking a yellow cell names the thieves — visible state, not just accuracy.
